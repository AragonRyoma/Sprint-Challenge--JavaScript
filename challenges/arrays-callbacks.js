...
let displays = zooAnimals.forEach((display) => {
  displayNames.push(
...
let lowCaseAnimalNames = zooAnimals.map((lCaps) => {
  return lCaps.animal_name.toLowerCase();
});
...
*/
let lowPopulationAnimals = zooAnimals.filter((lcount) => {
  return lcount.population < 5;
});
...
const populationTotal = zooAnimals.reduce(
  (sum, order) => sum + order.population,
  0
);
...
greeting = (fname, lname) => `Hello ${fname} ${lname}`;
